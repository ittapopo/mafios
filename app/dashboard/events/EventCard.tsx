...
export const EventCard: React.FC<EventCardProps> = ({ event, onClick }) => {
    ...
    // Calculate time since triggered (client-side only to avoid hydration mismatch)
    const [timeText, setTimeText] = useState('Nyss');
    ...
    useEffect(() => {
        const updateTime = () => {
            ...
            const timeSince = Math.floor((Date.now() - event.triggeredAt) / 1000 / 60);

            if (timeSince < 1) {
                setTimeText('Nyss');
            } else if (timeSince < 60) {
                setTimeText(`${timeSince}m sedan`);
            } else {
                setTimeText(`${Math.floor(timeSince / 60)}h sedan`);
            }
        };
